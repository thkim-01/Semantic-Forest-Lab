...
class OntologyRefinementGenerator:
    ...
    """
    def __init__(self, ontology_manager, static_refinements: Optional[List[OntologyRefinement]] = None, dl_profile: str = 'ALC'):
        ...
        self.allowed_ref_types = self._allowed_ref_types_for_profile(dl_profile)
    ...
    def _allowed_ref_types_for_profile(self, profile: str) -> List[str]:
        """Map DL profile names to allowed refinement/refinement-generators.

        This gives a practical way to approximate different DL expressivity
        by enabling/disabling certain candidate generation strategies.
        """
        profile = (profile or 'ALC').upper()
        mapping = {
            # EL: supports conjunctions and existential restrictions, but no universal
            'EL': ['concept', 'cardinality', 'domain', 'qualification', 'conjunction'],
            # AL: similar to EL for our generator (no explicit negation handled)
            'AL': ['concept', 'cardinality', 'domain', 'qualification', 'conjunction'],
            # ALC: most generators enabled (universal not explicitly generated)
            'ALC': ['concept', 'cardinality', 'domain', 'qualification', 'conjunction'],
        }
        return mapping.get(profile, mapping['ALC'])
