...
class LogicTreeNode(TreeNode):
    ...
    def __init__(self, instances: List, depth: int = 0, node_id: int = 0):
        ...
        # Note: We calculate unweighted gini here for initialization,
        # but Learner might recalculate weighted gini.
...
class LogicSDTLearner:
    ...
    def _calculate_gini_gain(
        ...
        parent_gini = self._calculate_gini(parent_node.instances)
        left_gini = self._calculate_gini(left_inst)
        right_gini = self._calculate_gini(right_inst)
